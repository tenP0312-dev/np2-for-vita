...
unsigned char YMF262Read(void *chipptr, int a) {
  ...
  if (chipbsd->m_data.timer_valid[0] &&
      !(chipbsd->m_data.reg_timerctrl & 0x40)) {
  ...
  if (chipbsd->m_data.timer_valid[1] &&
      !(chipbsd->m_data.reg_timerctrl & 0x20)) {
...
int YMF262FlagSave(void *chipptr, void *dstbuf) {
  ...
  buffer.insert(buffer.end(), (uint8_t *)&chipbsd->m_data,
...
int YMF262FlagLoad(void *chipptr, void *srcbuf, int size) {
  ...
  dummybuffer.insert(dummybuffer.end(), (uint8_t *)&chipbsd->m_data,
                     (uint8_t *)(&chipbsd->m_data + 1));
  ...
  memcpy(&chipbsd->m_data, (uint8_t *)srcbuf + fmbufsize,
         sizeof(chipbsd->m_data));
...
void YMF262UpdateOne(void *chipptr, INT16 **buffers, int length) {
  ...
  const double fmlengthf =
      chipbsd->m_data.fmcounter_rem +
      (double)length * chipbsd->m_data.fmrate / chipbsd->m_data.playrate;
  ...
  if (fmlength > 0) {
    ...
    if (hasdata) {
      for (int i = 0; i < length; i++) {
        int srcIndex = (int)((double)i * chipbsd->m_data.fmrate /
                             chipbsd->m_data.playrate);
