...
SINT32 vita_np2_cpu_mult = 0;
#if VITA_NP2_DIAG
extern UINT32 vita_cpu_diag_stos_hits;
extern UINT32 vita_cpu_diag_stos_iters;
extern UINT32 vita_cpu_diag_scas_hits;
extern UINT32 vita_cpu_diag_scas_iters;
extern UINT32 vita_cpu_diag_self_hits;
extern UINT32 vita_cpu_diag_self_iters;
...
int main(int argc, char *argv[])
{
    ...
    vita_np2_log("main: build vita-np2-kai-native-v036-fastpack 20260514");
    ...
    if (vita_np2_touhou_preset) {
        ...
        vita_np2_log("main: vita touhou preset on clk>=52x 56.4fps/44100hz");
    }
    if (vita_np2_cpu_mult > 0) {
        np2cfg.baseclock = 2457600;
        if (vita_np2_cpu_mult > CPU_MULTIPLE_MAX) {
            vita_np2_cpu_mult = CPU_MULTIPLE_MAX;
        }
        np2cfg.multiple = vita_np2_cpu_mult;
        vita_np2_log("main: vita cpu mult override");
    } else if (np2cfg.multiple < 16) {
        np2cfg.baseclock = 2457600;
        np2cfg.multiple = 16;
        vita_np2_log("main: vita cpu mult default min 16x");
    } else if (np2cfg.multiple > CPU_MULTIPLE_MAX) {
        np2cfg.multiple = CPU_MULTIPLE_MAX;
        vita_np2_log("main: vita cpu mult clamped max");
    }
    np2oscfg.DRAW_SKIP = vita_np2_frameskip;
    np2oscfg.NOWAIT = 1;
    {
        char vita_cfg_log[224];
        snprintf(vita_cfg_log, sizeof(vita_cfg_log), "main: vita cfg present_skip=%u core_drawskip=%u audio_light=%u speed_limit=%u touhou=%u cpu_mult_cfg=%ld clk=%ldx%ld hz=%u latency=%u skiplight=%d", vita_np2_frameskip, vita_np2_core_drawskip, vita_np2_audio_light, vita_np2_speed_limit, vita_np2_touhou_preset, (long)vita_np2_cpu_mult, (long)np2cfg.baseclock, (long)np2cfg.multiple, np2cfg.samplingrate, np2cfg.delayms, np2cfg.skiplight);
...
while (1) {
...
#if VITA_NP2_PROFILE
            vita_prof_begin(VITA_PROF_TASK);
#endif
            taskmng_rol();
#if VITA_NP2_PROFILE
            vita_prof_end(VITA_PROF_TASK);
#endif
            if (!task_avail) {
#if VITA_NP2_DIAG
            ...
            if (menuvram) {
#if VITA_NP2_PROFILE
                vita_prof_begin(VITA_PROF_PRESENT);
#endif
                scrnmng_gu_update();
#if VITA_NP2_PROFILE
                vita_prof_end(VITA_PROF_PRESENT);
                vita_prof_count_update(1);
                vita_prof_begin(VITA_PROF_WAIT);
#endif
                sceDisplayWaitVblankStart();
#if VITA_NP2_PROFILE
                vita_prof_end(VITA_PROF_WAIT);
                vita_prof_count_wait();
                vita_prof_report(vita_loop_counter, drawcount, np2oscfg.DRAW_SKIP, 1, pcstat.screenupdate, soundrenewal, CPU_CLOCK, CPU_BASECLOCK, CPU_REMCLOCK);
#endif
                vita_loop_counter++;
                continue;
            }
#if VITA_NP2_PROFILE
            {
                uint64_t vita_exec_start_us = vita_prof_now_us();
                vita_prof_begin(VITA_PROF_EXEC);
                pccore_exec(TRUE);
                vita_prof_end(VITA_PROF_EXEC);
                vita_prof_count_exec(TRUE);
                {
                    uint64_t vita_exec_dt = vita_prof_now_us() - vita_exec_start_us;
                    static UINT vita_slowexec_logs = 0;
                    if ((vita_exec_dt >= 100000ULL) && (vita_slowexec_logs < 128)) {
                        char vita_slowexec_buf[256];
                        snprintf(vita_slowexec_buf, sizeof(vita_slowexec_buf),
                                 "main-slowexec: us=%lu loop=%u draw=%u skp=%u cs=%04x eip=%08x clk=%u base=%u rem=%u ready=%u wait=%u",
                                 (unsigned long)vita_exec_dt,
                                 vita_loop_counter,
                                 drawcount,
                                 np2oscfg.DRAW_SKIP,
                                 (unsigned int)CPU_CS,
                                 (unsigned int)CPU_EIP,
                                 (unsigned int)CPU_CLOCK,
                                 (unsigned int)CPU_BASECLOCK,
                                 (unsigned int)CPU_REMCLOCK,
                                 (unsigned int)g_nevent.readyevents,
                                 (unsigned int)g_nevent.waitevents);
                        vita_np2_log(vita_slowexec_buf);
#if VITA_NP2_DIAG
                        vita_cpu_diag_log_all();
#endif
                        vita_slowexec_logs++;
                    }
                }
...
#if VITA_NP2_PROFILE
            vita_prof_report(vita_loop_counter, drawcount, np2oscfg.DRAW_SKIP, 0, pcstat.screenupdate, soundrenewal, CPU_CLOCK, CPU_BASECLOCK, CPU_REMCLOCK);
#endif
            }
            vita_loop_counter++;
            if (!task_avail) {
