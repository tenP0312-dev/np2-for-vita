...
static unsigned char vita_pg_vram[FRAMESIZE * 2] __attribute__((aligned(64)));
#define pg_vramtop ((char *)vita_pg_vram)
long pg_screenmode;
long pg_showframe;
...
static int pgaOutBlocking(unsigned long vol1, unsigned long vol2, void *buf)
{
    ...
    {
        int ret = sceAudioOutputPannedBlocking(pga_handle, vol1, vol2, buf);
#if defined(VITA_NP2_PORT)
        static int out_err_logs = 0;
        if ((ret < 0) && (out_err_logs < 16)) {
            char logbuf[96];
            snprintf(logbuf, sizeof(logbuf), "audio: output ret=0x%08x", ret);
            vita_np2_log(logbuf);
            out_err_logs++;
        }
#endif
...
int pgaInit(int samples)
{
...
#if defined(VITA_NP2_PORT)
        vita_np2_log("audio: ch reserve failed");
#endif
        return -1;
    }
#if defined(VITA_NP2_PORT)
    {
        char logbuf[96];
        snprintf(logbuf, sizeof(logbuf), "audio: pga init samples=%d handle=%d", samples, pga_handle);
        vita_np2_log(logbuf);
    }
#endif
#endif
