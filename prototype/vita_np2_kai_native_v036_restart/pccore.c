...
void pccore_exec(BOOL draw) {
...
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
  static UINT32 vita_pccore_slow_logs = 0;
  const uint64_t vita_pccore_start_us = vita_prof_now_us();
  UINT32 vita_pccore_loops = 0;
  UINT32 vita_pccore_cpu_calls = 0;
  UINT32 vita_pccore_event_calls = 0;
#endif
...
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
  vita_prof_begin(VITA_PROF_CORE_PRE);
#endif
...
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
  vita_prof_end(VITA_PROF_CORE_PRE);
#endif
  ...
  while (pcstat.screendispflag) {
...
#endif
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
    vita_prof_begin(VITA_PROF_CORE_EVENT);
#endif
    pic_irq();
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
    vita_prof_end(VITA_PROF_CORE_EVENT);
    vita_pccore_event_calls++;
#endif
#if defined(SUPPORT_IA32_HAXM)
    if (CPU_RESETREQ &&
...
#endif
    {
      ...
      if (CPU_REMCLOCK > 0) {
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
        vita_prof_begin(VITA_PROF_CORE_CPU);
#endif
...
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
        vita_prof_end(VITA_PROF_CORE_CPU);
        vita_pccore_cpu_calls++;
#endif
      }
#else
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
...
#endif
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
    vita_prof_begin(VITA_PROF_NEVT_STEP);
#endif
    nevent_progress();
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
    vita_prof_end(VITA_PROF_NEVT_STEP);
#endif
#if defined(SUPPORT_ASYNC_CPU)
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
    vita_prof_begin(VITA_PROF_CORE_EVENT);
...
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
  vita_prof_begin(VITA_PROF_CORE_POST);
#endif
#if defined(SUPPORT_ASYNC_CPU)
  asynccpu_lateflag = 0;
  asynccpu_fastflag = 0;
...
#if defined(VITA_NP2_PORT) && VITA_NP2_PROFILE
  vita_prof_end(VITA_PROF_CORE_POST);
  {
    const uint64_t vita_pccore_dt = vita_prof_now_us() - vita_pccore_start_us;
    if ((vita_pccore_dt >= 100000ULL) && (vita_pccore_slow_logs < 128)) {
      char vita_pccore_buf[256];
      snprintf(vita_pccore_buf, sizeof(vita_pccore_buf),
               "pccore-slow: us=%lu loops=%u cpu=%u evt=%u draw=%d scr=%d cs=%04x eip=%08x clk=%u base=%u rem=%u ready=%u wait=%u",
               (unsigned long)vita_pccore_dt,
               vita_pccore_loops,
               vita_pccore_cpu_calls,
               vita_pccore_event_calls,
               draw,
      ...
      vita_np2_log(vita_pccore_buf);
      vita_pccore_slow_logs++;
    }
  }
#endif
