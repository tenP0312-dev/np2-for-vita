...
const SDRAWFN	*sdrawfn;
	...
	UINT8		force_draw;
#if defined(VITA_NP2_PORT)
	UINT		vita_dirty_lines;
#endif

	force_draw = (redraw || redrawpending);
	if (force_draw) {
		updateallline(0x80808080);
		redrawpending = 0;
	}
	...
	for (i=0; i<SURFACE_HEIGHT; i++) {
		if (renewal_line[i] & bit) {
			vita_dirty_lines++;
		}
	}
	if (vita_np2_core_drawskip && !force_draw && (vita_dirty_lines >= 96)) {
		static UINT vita_core_draw_counter = 0;
		UINT period = (UINT)vita_np2_frameskip + 1;
		if ((period > 1) && ((vita_core_draw_counter++ % period) != 0)) {
			return(0);
		}
	}
#endif
