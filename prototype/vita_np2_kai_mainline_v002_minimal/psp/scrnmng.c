...
static UINT16 vita_full640_buf[640 * 400] __attribute__((aligned(64)));
static int vita_present_valid = 0;
static int vita_full640_active = 0;
#endif
                           ...
                           UINT first_event, SINT32 first_clock)
{
    ...
    snprintf(buf, sizeof(buf),
             "diagkai1: loop=%u draw=%u(+%u) real=%lu cpu=%lu(+%lu) base=%lu rem=%ld pc=%04x:%08lx%s ev=%u/%u first=%u:%ld out=%08lx%s np2=%08lx/%08lx%s upd=%08lx/%u ren=%08lx/%u dirty=%d scr=%u snd=%d",
             loop_count,
             draw_count,
...
void scrnmng_gu_update()
{
    ...
    const UINT16 *present_full = vita_full640_active ? vita_full640_buf : NULL;
...
#if defined(VITA_NP2_PORT)
        (void)surf;
        /* Vita presents vita_full640_buf directly in the main loop. */
        vita_scrn_dirty = 1;
#else
        scrnmng_gu_update();
#endif
