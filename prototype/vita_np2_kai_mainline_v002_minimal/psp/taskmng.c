...
#if defined(VITA_NP2_PORT)
typedef struct vita_touch_state_s {
    int down;
    int prev_down;
    int x;
    int y;
...
static vita_touch_state_t vita_touch;
static SceTouchPanelInfo vita_touch_info;
static int vita_touch_info_ready = 0;

static void vita_input_log(const char *tag)
{
    char buf[128];
    snprintf(buf, sizeof(buf), "input: %s touch=%d x=%d y=%d", tag,
             vita_touch.down, vita_touch.x, vita_touch.y);
    vita_np2_log(buf);
}

static void vita_touch_init(void)
{
    memset(&vita_touch, 0, sizeof(vita_touch));
    sceTouchSetSamplingState(SCE_TOUCH_PORT_FRONT, SCE_TOUCH_SAMPLING_STATE_START);
    if (sceTouchGetPanelInfo(SCE_TOUCH_PORT_FRONT, &vita_touch_info) >= 0) {
        vita_touch_info_ready = 1;
        vita_np2_log("input: front touch ready");
    }
    else {
        vita_touch_info_ready = 0;
        vita_np2_log("input: front touch panel info failed; using fallback scale");
    }
}
...
static void vita_touch_poll(void)
{
    ...
    vita_touch.prev_down = vita_touch.down;
    vita_touch.prev_x = vita_touch.x;
    vita_touch.prev_y = vita_touch.y;
    ...
    if (vita_touch_info_ready) {
        txmax = vita_touch_info.maxAaX - vita_touch_info.minAaX;
        tymax = vita_touch_info.maxAaY - vita_touch_info.minAaY;
        vita_touch.x = ((touch.report[0].x - vita_touch_info.minAaX) * 480) / (txmax ? txmax : 1);
        vita_touch.y = ((touch.report[0].y - vita_touch_info.minAaY) * 272) / (tymax ? tymax : 1);
    }
    else {
        vita_touch.x = (touch.report[0].x * 480) / txmax;
        vita_touch.y = (touch.report[0].y * 272) / tymax;
    }

    if (vita_touch.x < 0) vita_touch.x = 0;
    if (vita_touch.x > 479) vita_touch.x = 479;
    if (vita_touch.y < 0) vita_touch.y = 0;
    if (vita_touch.y > 271) vita_touch.y = 271;
    vita_touch.down = 1;
}
#endif
...
{
#if defined(VITA_NP2_PORT)
    vita_np2_log("taskmng_exit: ignored on vita");
    task_avail = TRUE;
#else
    task_avail = FALSE;
#endif
...
int taskmng_onscrn_active(void)
{
    return (onscrn_cnt > 0 || kc_scrn_cnt > 0);
}
...
#if defined(VITA_NP2_PORT)
    if (vita_touch.down) {
        if (vita_touch.prev_down) {
            x += (vita_touch.x - vita_touch.prev_x) / 2;
            y += (vita_touch.y - vita_touch.prev_y) / 2;
        }
        else {
            vita_input_log("pc98 touch down");
        }
    }
#endif

    mousemng_update(x, y,
                    (pad->Buttons & mbutton_l)
#if defined(VITA_NP2_PORT)
                    || vita_touch.down
#endif
                    , pad->Buttons & mbutton_r);
}
...
static int pspmenu_mode(UINT32 beingdown, UINT32 beingup, SceCtrlData *pad)
{
    ...
    if (vita_touch.down) {
        if (!vita_touch.prev_down) {
            vita_input_log("menu touch down");
        }
        if (!vita_touch.prev_down || x != vita_touch.x || y != vita_touch.y) {
            x = vita_touch.x;
            y = vita_touch.y;
            moving = TRUE;
        }
...
#if defined(VITA_NP2_PORT)
    if (vita_touch.down && !vita_touch.prev_down) {
        if (menuvram) {
...
#if defined(VITA_NP2_PORT)
    static int vita_end_hold_frames = 0;
#endif
    static enum {
        pspmenu,
        pc98mouse,
...
#if defined(VITA_NP2_PORT)
    if (vita_end_hold_frames > 0) {
        vita_end_hold_frames--;
        if (vita_end_hold_frames == 0) {
            vita_np2_log("input: END hold up");
            keystat_keyup(0x3f);
        }
    }
#endif
...
#if defined(VITA_NP2_PORT)
    if (menuvram && mouse_mode != pspmenu) {
...
//    sceCtrlRead(&psppad, 1);
...
#if defined(VITA_NP2_PORT)
    if (((beingdown & PSP_CTRL_START) && (button_down & PSP_CTRL_SELECT)) ||
        ((beingdown & PSP_CTRL_SELECT) && (button_down & PSP_CTRL_START))) {
        if (vita_end_hold_frames != 0) {
            vita_np2_log("input: END reset hold extend");
        }
        pccore_reset();
        vita_np2_log("input: reset for END hold");
        vita_end_hold_frames = 180;
        vita_np2_log("input: END reset hold down");
        keystat_keydown(0x3f);
        vita_np2_log("input: reset with END hold");
    ...
    if (beingdown & PSP_CTRL_SELECT) {
        if (mouse_mode != keyconf) {
            ...
            mouse_mode = keyconf;
#if defined(VITA_NP2_PORT)
            vita_np2_log("input: mode keyconf");
#endif
            onscrn_cnt = 0;
            kc_scrn_cnt = 1;
    ...
    if (beingdown & PSP_CTRL_LTRIGGER) {
        if (mouse_mode != pspmenu) {
            ...
            mouse_mode = pspmenu;
#if defined(VITA_NP2_PORT)
            vita_np2_log("input: mode pspmenu");
#endif
    ...
    if (beingdown & PSP_CTRL_RTRIGGER) {
        if (mouse_mode != pc98skbd) {
            ...
            mouse_mode = pc98skbd;
#if defined(VITA_NP2_PORT)
            vita_np2_log("input: mode pc98skbd");
#endif
