...
def main() -> int:
    ap = argparse.ArgumentParser(description="Recursively normalize line endings to LF.")
    ...
    for f in iter_files(args.dir, opt):
        ...
        new = normalize_newlines(text)
        if new == text:
            rep.noop(f, "already LF")
            continue

        out = new.encode("utf-8")
        # Preserve BOM if present in input
        if raw.startswith(b"\xef\xbb\xbf"):
            out = b"\xef\xbb\xbf" + out

        if args.dry_run:
            rep.ok(f, "would normalize LF")
            continue

        try:
            safe_write_bytes(f, out)
            rep.ok(f, "normalized LF")
        except PermissionError:
            rep.skip(f, "permission")
        except Exception:
