...
/*! ルート情報 */
static const HDRVFILE s_hddroot = {
    {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
    0,
    0,
    0x10,
 ...
 * @param[in] lpPath パス
 */
static void RealPath2FcbSub(char *lpFcbname, UINT cchFcbname,
                            const char *lpPath) {
  REG8 c;
 ...
 * @return ファイル一覧
 */
LISTARRAY hostdrvs_getpathlist(const HDRVPATH *phdp, const char *lpMask,
                               UINT nAttr) {
  LISTARRAY ret;
  LISTARRAY lst;
  HDRVFILE file;
  ...
  if (ret != NULL) {
    ...
    if (phdp->file.attr & 0x10) {
      ...
      if (!isRoot && IsMatchFcb(&file, lpMask, nAttr)) {
        ...
        if (hdd != NULL) {
          hdd->file = file;
          file_cpyname(hdd->szFilename, OEMTEXT("."),
                       NELEMENTS(hdd->szFilename));
        }
      }
    ...
    if (flh != FLISTH_INVALID) {
      do {
        ...
        if ((file.fcbname[0] == ' ') ||
            (listarray_enum(lst, IsMatchName, file.fcbname) != NULL)) {
          continue;
        }
 ...
 * @return 次の DOS パス
 */
static const char *DosPath2FcbSub(char *lpFcbname, UINT cchFcbname,
...
UINT hostdrvs_getrealdir(HDRVPATH *phdp, char *lpFcbname,
  ...
  while (TRUE /*CONSTCOND*/) {
    ...
    if ((FindSinglePath(phdp, lpFcbname) != SUCCESS) ||
        ((phdp->file.attr & 0x10) == 0)) {
      return FAILURE;
    }
    lpDosPath++;
  }
...
UINT hostdrvs_getrealpath(HDRVPATH *phdp, const char *lpDosPath) {
  ...
  if (lpDosPath[0] == '\0' ||
      (lpDosPath[0] == '\\' && lpDosPath[1] == '\0')) // root check
  {
    file_cpyname(phdp->szPath, np2cfg.hdrvroot,
                 NELEMENTS(phdp->szPath)); // copy root path
    return ERR_NOERROR;
  }
