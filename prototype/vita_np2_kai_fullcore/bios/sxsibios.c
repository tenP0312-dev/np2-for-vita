  ...
  if ((oldAL & 0xf0) == 0x80) {
    ...
    if (CPU_STAT_PM && CPU_STAT_VM86 && biosioemu.enable) {
      ...
      if (sxsi->totals > 0xffffff) {
        ...
        biosioemu_push8(0x64c, 0xA0 | ((sxsi_unittbl[oldAL & 0x3] & 0x1) << 4) |
                                   (hd & 0x0f));
...
static REG8 sasibios_init(UINT type, SXSIDEV sxsi) {
  ...
  for (i = 0x00, bit = 0x0100; i < 0x02; i++, bit <<= 1) {
    ...
    if ((sxsi) &&
        ((sxsi->flag & SXSIFLAG_READY) && sxsi->devtype == SXSIDEV_HDD)) {
      diskequip |= bit;
    }
  }
...
static REG8 sasibios_sense(UINT type, SXSIDEV sxsi) {
  ...
  if (type == SXSIBIOS_SASI) {
    ...
    if (sxsi->sectors == 33 && sxsi->surfaces <= 8) {
      ...
      if (lbasize < 10475520)
        return 0; // 10MB未満　4*33*310
      else if (lbasize < 15713280)
        return 1; // 15MB未満　6*33*310
      else if (lbasize < 20275200)
        return 2; // 20MB未満　8*33*310 →thdの20MBは8:33:300なので調整
      else if (lbasize < 41564160)
        return 3; // 40MB未満　8*33*615
      else if (lbasize == 41564160)
        return 4; // 40MB 8*33*615
      else
        return 0x0f;
    } else {
      return 0x0f;
...
static void scsibios_set(REG8 drv, REG8 sectors, REG8 surfaces, REG16 cylinders,
                         REG16 size, BOOL hwsec) {
...
static REG8 scsibios_init(UINT type, SXSIDEV sxsi) {
  ...
  for (i = 0, bit = 1; i < 4; i++, bit <<= 1) {
    ...
    if ((sxsi) && (sxsi->flag & SXSIFLAG_READY)) {
      ...
      scsibios_set(i, sxsi->sectors, sxsi->surfaces, sxsi->cylinders,
                   sxsi->size, TRUE);
    }
  }
  (void)type;
