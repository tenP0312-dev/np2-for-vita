...
static const _XDFINFO supportxdf[] = {
...
#if 1
    // 512
    {0, 160, 15, 2, DISKTYPE_2HD,
     0}, //	BKDSK(HD5)	MS-DOS 1.21M(2HC)
#endif
    // 1024
    {0, 154, 8, 3, DISKTYPE_2HD, 0}, //	XDF他2HD、BIN、FLP等
                                     //	BKDSK(HDM)	MS-DOS 1.25M(2HD)
    // 1.44MB
    {0, 160, 18, 2, DISKTYPE_2HD,
     1},                              //	BKDSK(HD4)	MS-DOS 1.44M(2HD)
    {0, 160, 21, 2, DISKTYPE_2HD, 1}, //	BKDSK     	MS 2HD-DMF

    //	追加でいくつかのBKDSK形式に対応
    {0, 154, 26, 1, DISKTYPE_2HD,
     0}, //	BKDSK(H01)	2HD:256byte*26sec (0-154)track
    ...
    {256, 154, 8, 3, DISKTYPE_2HD,
     0}, //	DIP			2HD-8セクタ(1.25MB)
         //	FIM			2HD-8セクタ(1.25MB)
    {256, 154, 26, 1, DISKTYPE_2HD,
     0}, //	FIM			2HD-26セクタ
...
BRESULT fdd_set_xdf(FDDFILE fdd, FDDFUNC fdd_fn, const OEMCHAR *fname, int ro) {
  ...
  while (xdf < (supportxdf + NELEMENTS(supportxdf))) {
    ...
    if (size == fdsize) {
      ...
      fdd_fn->formatinit =
          fdd_formatinit_xdf; /* 170107 to support format command */
...
BRESULT fdd_set_fdi(FDDFILE fdd, FDDFUNC fdd_fn, const OEMCHAR *fname, int ro) {
  ...
  if (((size & (size - 1)) != 0) || (!(size & 0x7f80)) || (sectors == 0) ||
      (sectors >= 256) || (surfaces != 2) || (cylinders == 0) ||
      (cylinders >= 128)) {
    return (FAILURE);
  }
  ...
  fdd_fn->writeid = fdd_dummy_xxx;
  fdd_fn->formatinit =
      fdd_formatinit_xdf; /* 170107 modified to work on Windows 9x/2000 */
  fdd_fn->formating = fdd_formating_xxx;
  fdd_fn->isformating = fdd_isformating_xxx;
  //
  return (SUCCESS);
...
/* 実装補足
   先頭からのオフセット値は単純にトラック数ｘ1トラック当たりのセクタ数ｘセクタサイズ
...
BRESULT fddxdf_diskaccess(FDDFILE fdd) {
  ...
  if ((fdd->type != DISKTYPE_BETA) || (CTRL_FDMEDIA != fdd->inf.xdf.disktype)) {
    return (FAILURE);
  }
  return (SUCCESS);
}
