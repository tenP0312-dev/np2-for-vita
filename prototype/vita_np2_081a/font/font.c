...
const UINT8	*p;
...
#if defined(VITA_NP2_PORT)
			vita_np2_log("font: make font.tmp begin");
#endif
			makepc98bmp(fname);
		}
		loading = fontpc98_read(fname, loading);
	...
	{
		char logbuf[96];
		snprintf(logbuf, sizeof(logbuf), "font: load type=%u remain=0x%02x", type, loading);
		vita_np2_log(logbuf);
		if (loading & FONTLOAD_KNJ) {
			vita_np2_log("font: missing kanji font; put FONT.ROM or FONT.BMP in ux0:data/np2vita");
		}
	}
#endif
	return(type);
