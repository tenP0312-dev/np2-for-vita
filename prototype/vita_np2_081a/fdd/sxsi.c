...
BRESULT sxsi_devopen(REG8 drv, const OEMCHAR *fname) {
...
#if defined(VITA_NP2_PORT)
	char logbuf[96];
	OEMSPRINTF(logbuf, OEMTEXT("sxsi_devopen: drv=%02x begin"), drv);
	vita_np2_log(logbuf);
	vita_np2_log((fname) ? fname : "sxsi_devopen: fname=(null)");
#endif
	SXSIDEV		sxsi;
	BRESULT		r;
	...
	if (r != SUCCESS) {
#if defined(VITA_NP2_PORT)
		vita_np2_log("sxsi_devopen: device open failed");
#endif
		goto sxsiope_err;
	}
...
#endif
#if defined(VITA_NP2_PORT)
	vita_np2_log("sxsi_devopen: success");
#endif
	return(SUCCESS);

sxsiope_err:
#if defined(VITA_NP2_PORT)
	vita_np2_log("sxsi_devopen: failure");
#endif
	return(FAILURE);
}
