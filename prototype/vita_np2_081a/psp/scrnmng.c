                           ...
                           UINT first_event, SINT32 first_clock)
{
    ...
    snprintf(buf, sizeof(buf),
             "diagv17: loop=%u draw=%u(+%u) cpu=%lu(+%lu) pc=%04x:%08lx%s ev=%u/%u first=%u:%ld out=%08lx%s raw=%08lx/%08lx%s np2=%08lx/%08lx%s upd=%08lx/%u ren=%08lx/%u pcm=f%02x d%02x v%ld r%ld fs%ld rq%u iq%u io=466:%lu/%02x 468:%lu/%02x w46c:%lu/%02x pic=%02x:%02x:%02x/%02x:%02x:%02x dirty=%d scr=%u snd=%d",
             loop_count,
             draw_count,
