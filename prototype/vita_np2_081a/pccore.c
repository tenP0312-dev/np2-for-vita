...
#if defined(VITA_NP2_PORT)
	UINT32 vita_loop_guard;
	static UINT vita_watchdog_logs = 0;
#endif
	drawframe = draw;
//	keystat_sync();
	soundmng_sync();
	...
	screendispflag = 1;
#if defined(VITA_NP2_PORT)
	/* Zero wait states on Vita: GRCG/VRAM waits waste cycles without ARM work.
	   Games complete graphics operations in fewer PC-98 frames → faster. */
	MEMWAIT_TRAM = 0;
	MEMWAIT_VRAM = 0;
	MEMWAIT_GRCG = 0;
#else
...
#if defined(VITA_NP2_PORT)
		if (++vita_loop_guard > 200000) {
			if (vita_watchdog_logs < 16) {
				vita_np2_log("pccore_exec: watchdog break");
				vita_watchdog_logs++;
			}
			screendispflag = 0;
			break;
