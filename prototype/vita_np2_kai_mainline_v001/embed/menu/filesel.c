...
extern int vita_np2_autorun_trace;
#endif
...
#if defined(VITA_NP2_PORT)
static void filesel_log_hex(const char *tag, const OEMCHAR *name)
{
    char buf[192];
    char *p;
    int i;
    ...
    p = buf;
    p += snprintf(p, sizeof(buf), "%s", tag);
    for (i = 0; (i < 24) && (name[i] != '\0') && ((p - buf) < (int)sizeof(buf) - 4); i++) {
        p += snprintf(p, sizeof(buf) - (p - buf), " %02x", (unsigned int)(UINT8)name[i]);
    }
    vita_np2_log(buf);
}
#endif
