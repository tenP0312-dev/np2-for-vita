...
UINT8 vita_np2_force_nowait = 0;
UINT8 vita_np2_force_skipline = 1;
UINT8 vita_np2_force_gdc = 0;
UINT8 vita_np2_sound_mode = 0;
UINT8 vita_np2_sound_board = 0xff;
UINT8 vita_np2_use_fmgen = 255;
#endif
...
int main(int argc, char *argv[])
{
    ...
    vita_np2_log("main: build vita-np2-kai-mainline-v001 20260516");
    ...
    if (vita_np2_force_gdc > 1) {
        vita_np2_force_gdc = 1;
    }
    if (vita_np2_force_skipline > 1) {
        vita_np2_force_skipline = 1;
    }
    if (vita_np2_force_nowait > 1) {
        vita_np2_force_nowait = 1;
    }
    if (vita_np2_sound_mode > 2) {
        vita_np2_sound_mode = 2;
    }
    if (vita_np2_touhou_preset) {
        np2cfg.baseclock = 2457600;
        if (np2cfg.multiple < 12) {
            np2cfg.multiple = 12;
        }
        ...
        if (vita_np2_frameskip < 3) {
            vita_np2_frameskip = 3;
        }
        vita_np2_core_drawskip = 0;
        vita_np2_audio_light = 1;
        vita_np2_speed_limit = 0;
        vita_np2_log("main: vita touhou preset light clk>=12x/11025hz");
    }
    ...
    if (vita_np2_force_nowait) {
        np2oscfg.NOWAIT = 1;
        vita_np2_log("main: vita force nowait=1");
    }
    if (vita_np2_sound_board != 0xff) {
        np2cfg.SOUND_SW = vita_np2_sound_board;
        vita_np2_log("main: vita sound board override");
    }
    if (vita_np2_use_fmgen != 255) {
        vita_np2_log("main: vita use fmgen ignored in nofmgen build");
    }
    if (vita_np2_sound_mode == 1) {
        vita_np2_audio_light = 1;
        vita_np2_log("main: vita sound mode light");
    } else if (vita_np2_sound_mode == 2) {
        vita_np2_audio_light = 1;
        np2cfg.SOUND_SW = 0;
        np2cfg.vol_fm = 0;
        np2cfg.vol_ssg = 0;
        ...
        np2cfg.BEEP_VOL = 0;
        np2cfg.MOTOR = 0;
        vita_np2_log("main: vita sound mode off");
    }
    ...
    if (vita_np2_force_skipline && !np2cfg.skipline) {
        np2cfg.skipline = 1;
        vita_np2_log("main: vita force skipline=1");
    }
    if (vita_np2_force_gdc && np2cfg.dipsw[1] != 0xf3) {
        np2cfg.dipsw[1] = 0xf3;
        vita_np2_log("main: vita force dipsw gdc=2.5mhz");
    }
    {
        char vita_cfg_log[384];
        snprintf(vita_cfg_log, sizeof(vita_cfg_log), "main: vita cfg draw_skip=%u core_drawskip=%u audio_light=%u speed_limit=%u touhou=%u cpu_mult_cfg=%ld clk=%ldx%ld hz=%u latency=%u skipline=%u skiplight=%d nowait=%u force_gdc=%u upd72020=%u grcg=%u sound_mode=%u sound_sw=%02x fmgen=%u",
            vita_np2_frameskip, vita_np2_core_drawskip, vita_np2_audio_light, vita_np2_speed_limit, vita_np2_touhou_preset, (long)vita_np2_cpu_mult,
            (long)np2cfg.baseclock, (long)np2cfg.multiple, np2cfg.samplingrate, np2cfg.delayms, np2cfg.skipline, np2cfg.skiplight,
            np2oscfg.NOWAIT, vita_np2_force_gdc, np2cfg.uPD72020, np2cfg.grcg, vita_np2_sound_mode, np2cfg.SOUND_SW, vita_np2_use_fmgen);
