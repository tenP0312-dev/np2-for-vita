...
#define PSP_KEY_TXT "ux0:data/np2vita/psp_key.txt"
#else
...
static KEYNAME *_search_kn(UINT8 n, char *s)
{
    ...
    end = (KEYNAME *)s_keyname + sizeof(s_keyname)/sizeof(KEYNAME);


    for (kn = (KEYNAME *)s_keyname; kn != end; kn++) {
        if (n == 0xff && s != NULL && strcmp(kn->str, s) == 0) {
            return kn;
        }
        if (s == NULL && kn->num == n) {
            return kn;
...
char *kc_get_keystrs(int ksnum, int tokenid)
{
    ...
    static char keystrs[7 * KC_MAXKEYDEF + 1];
...
#if defined(VITA_NP2_PORT)
static const char vita_default_psp_key[] =
    "[KeySetting]\r\n"
    "up = UP\r\n"
    "down = DOWN\r\n"
    "left = LEFT\r\n"
    ...
    "mm_triangle = SHIFT\r\n"
    "mm_square = SPC\r\n"
    "comment = \"Ten-key move\"\r\n";

static void vita_ensure_default_keyconf(void)
{
    FILE *fd = fopen(PSP_KEY_TXT, "r");
    if (fd != NULL) {
        fclose(fd);
        return;
    }
    fd = fopen(PSP_KEY_TXT, "w");
    if (fd != NULL) {
        fputs(vita_default_psp_key, fd);
        fclose(fd);
    }
}
#endif
