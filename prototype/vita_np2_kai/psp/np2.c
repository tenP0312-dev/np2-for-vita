...
int main(int argc, char *argv[])
{
    ...
    vita_np2_log("main: build vita-np2-kai-sysport-v7 20260506");
...
#if defined(VITA_NP2_PORT)
    {
        ...
        while (1) {
            ...
            pccore_exec(TRUE);
            if (drawcount == vita_last_drawcount) {
                vita_draw_stall++;
            }
            else {
                vita_last_drawcount = drawcount;
            ...
            if ((vita_loop_counter != 0) && ((vita_loop_counter % 600) == 0)) {
                scrnmng_vita_diag_log(vita_loop_counter, drawcount, pccore.realclock, screenupdate, soundrenewal, CPU_CLOCK, CPU_BASECLOCK, CPU_REMCLOCK, CPU_CS, CPU_EIP, nevent.readyevents, nevent.waitevents, nevent.readyevents ? nevent.level[0] : 99, nevent.readyevents ? nevent.item[nevent.level[0]].clock : 0);
            }
            scrnmng_gu_update();
            sceDisplayWaitVblankStart();
            vita_loop_counter++;
