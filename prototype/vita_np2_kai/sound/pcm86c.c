...
static const UINT clk20_128[] = {
	...
	PCM86CFG	pcm86cfg;
static UINT32 datawriteirqwait = 0;
#if defined(VITA_NP2_PORT)
UINT32 vita_pcm86_kai_intrq_count = 0;
UINT32 vita_pcm86_kai_cb_irq_count = 0;
UINT32 vita_pcm86_kai_force_irq_count = 0;
#endif
...
static void pcm86_setwaittime(void) {
...
void pcm86_cb(NEVENTITEM item) {
	...
	if (pcm86.reqirq) {
		...
		if ((pcm86.virbuf <= pcm86.fifosize) ||
			((pcm86.realbuf > (SINT32)pcm86.stepmask) && (adjustbuf <= pcm86.fifosize))) {
			pcm86.reqirq = 0;
			pcm86.irqflag = 1;
#if defined(VITA_NP2_PORT)
			vita_pcm86_kai_cb_irq_count++;
#endif
...
void SOUNDCALL pcm86gen_checkbuf(void) {
	...
	if (bufs < 0) {
		...
		if (pcm86.virbuf <= pcm86.fifosize) {
			...
			pcm86.irqflag = 1;
#if defined(VITA_NP2_PORT)
			vita_pcm86_kai_force_irq_count++;
#endif
			pic_setirq(fmtimer.irq);
		}
		else {
		...
		if ((pcm86.virbuf > pcm86.fifosize) && (pcm86.realbuf > (SINT32)pcm86.stepmask) &&
			(pcm86.realbuf > pcm86.virbuf + pcm86.fifosize * 3)) {
		...
		if (bufs > 0) {
			bufs &= ~3;
			pcm86.realbuf -= bufs;
			pcm86.readpos = (pcm86.readpos + bufs) & PCM86_BUFMSK;
		}
	}
}
...
BOOL pcm86gen_intrq(void) {
	...
	curclk = CPU_CLOCK + CPU_BASECLOCK - CPU_REMCLOCK;
	if (!(pcm86.fifo & 0x20) || (curclk - pcm86.lastclockforwait < datawriteirqwait)) {
		return(FALSE);
	}
	if ((pcm86.reqirq) &&
		((pcm86.virbuf <= pcm86.fifosize) ||
		 ((pcm86.realbuf > (SINT32)pcm86.stepmask) && (pcm86.realbuf <= pcm86.fifosize)))) {
		pcm86.reqirq = 0;
		pcm86.irqflag = 1;
#if defined(VITA_NP2_PORT)
		vita_pcm86_kai_intrq_count++;
#endif
		return(TRUE);
	}
	return(FALSE);
