...
static VITA_IOLOG vita_iolog[64];
static UINT vita_iolog_pos = 0;
static UINT32 vita_io_in_count[256];
static UINT32 vita_io_out_count[256];
static UINT16 vita_io_last_port[256];
static UINT32 vita_io_last_val[256];

static void vita_iolog_record(UINT port, UINT value, UINT flags) {
	UINT idx = vita_iolog_pos & 63;
	UINT bucket = port & 0xff;
	vita_iolog[idx].port = (UINT16)port;
	vita_iolog[idx].value = (UINT32)value;
	vita_iolog[idx].flags = (UINT8)flags;
	vita_iolog[idx].cs = CPU_CS;
	vita_iolog[idx].eip = CPU_EIP;
	vita_iolog_pos++;
	if (flags & 1) {
		vita_io_in_count[bucket]++;
	}
	else {
		vita_io_out_count[bucket]++;
	}
	vita_io_last_port[bucket] = (UINT16)port;
	vita_io_last_val[bucket] = (UINT32)value;
}
...
void iocore_vita_diag_log_recent(void) {
	...
	UINT i;
	UINT start;
	UINT best_in[4] = {0, 0, 0, 0};
	UINT best_out[4] = {0, 0, 0, 0};

	for (i=0; i<256; i++) {
		UINT j;
		for (j=0; j<4; j++) {
			if (vita_io_in_count[i] > vita_io_in_count[best_in[j]]) {
				UINT k;
				for (k=3; k>j; k--) {
					best_in[k] = best_in[k-1];
				}
				best_in[j] = i;
				break;
			}
		...
		for (j=0; j<4; j++) {
			if (vita_io_out_count[i] > vita_io_out_count[best_out[j]]) {
				UINT k;
				for (k=3; k>j; k--) {
					best_out[k] = best_out[k-1];
				}
				best_out[j] = i;
				break;
			}
	...
	snprintf(buf, sizeof(buf),
		"iolog: hot in %04x:%lu/%08lx %04x:%lu/%08lx %04x:%lu/%08lx %04x:%lu/%08lx out %04x:%lu/%08lx %04x:%lu/%08lx %04x:%lu/%08lx %04x:%lu/%08lx pos=%u",
		vita_io_last_port[best_in[0]], (unsigned long)vita_io_in_count[best_in[0]], (unsigned long)vita_io_last_val[best_in[0]],
		vita_io_last_port[best_in[1]], (unsigned long)vita_io_in_count[best_in[1]], (unsigned long)vita_io_last_val[best_in[1]],
		vita_io_last_port[best_in[2]], (unsigned long)vita_io_in_count[best_in[2]], (unsigned long)vita_io_last_val[best_in[2]],
		vita_io_last_port[best_in[3]], (unsigned long)vita_io_in_count[best_in[3]], (unsigned long)vita_io_last_val[best_in[3]],
		vita_io_last_port[best_out[0]], (unsigned long)vita_io_out_count[best_out[0]], (unsigned long)vita_io_last_val[best_out[0]],
		vita_io_last_port[best_out[1]], (unsigned long)vita_io_out_count[best_out[1]], (unsigned long)vita_io_last_val[best_out[1]],
		vita_io_last_port[best_out[2]], (unsigned long)vita_io_out_count[best_out[2]], (unsigned long)vita_io_last_val[best_out[2]],
		vita_io_last_port[best_out[3]], (unsigned long)vita_io_out_count[best_out[3]], (unsigned long)vita_io_last_val[best_out[3]],
		vita_iolog_pos);
	vita_np2_log(buf);

	start = (vita_iolog_pos > 16) ? (vita_iolog_pos - 16) : 0;
	for (i=start; i<vita_iolog_pos; i++) {
		VITA_IOLOG *l = &vita_iolog[i & 63];
		snprintf(buf, sizeof(buf), "iolog: recent %u %c%d port=%04x val=%08lx pc=%04x:%08lx",
			i, (l->flags & 1) ? 'I' : 'O', (l->flags >> 1), l->port, (unsigned long)l->value, l->cs, (unsigned long)l->eip);
		vita_np2_log(buf);
	}
}
#endif
...
REG16 IOINPCALL iocore_inp16(UINT port) {
...
#if defined(SUPPORT_IDEIO)
	if (port == 0x0640) {
		REG16 vret = ideio_r16(port);
#if defined(VITA_NP2_PORT)
		vita_iolog_record(port, vret, 3);
#endif
		return(vret);
	}
#endif
	if ((port & 0xfffc) == 0x005c) {
		REG16 vret = artic_r16(port);
#if defined(VITA_NP2_PORT)
		vita_iolog_record(port, vret, 3);
#endif
		return(vret);
	...
	if (!(port & 0x0c00)) {
		switch(ioterminate[port & 0xff]) {
			case TERM_WORD:
#if defined(VITA_NP2_PORT)
				vita_iolog_record(port, WORD_TERMINATE, 3);
#endif
			...
			case TERM_ACTIVE:
				ret = iof->ioinp[port & 0xff](port);
#if defined(VITA_NP2_PORT)
				vita_iolog_record(port, ((CPU_AX & 0xff00) + ret), 3);
#endif
			...
			case TERM_PLUS:
				ret = iof->ioinp[port & 0xff](port);
#if defined(VITA_NP2_PORT)
				vita_iolog_record(port, (0xff00 + ret), 3);
#endif
				return(0xff00 + ret);
			...
			case TERM_EXT08:
				ret = iof->ioinp[port & 0xff](port);
#if defined(VITA_NP2_PORT)
				vita_iolog_record(port, (0x0800 + ret), 3);
#endif
				return(0x0800 + ret);
		}
	}
	...
	iof = iocore.base[(port >> 8) & 0xff];
	{
		REG16 vret = (UINT16)((iof->ioinp[port & 0xff](port) << 8) + ret);
#if defined(VITA_NP2_PORT)
		vita_iolog_record(port - 1, vret, 3);
#endif
		return(vret);
	}
}
