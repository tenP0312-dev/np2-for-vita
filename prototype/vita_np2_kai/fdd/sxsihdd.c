...
#if defined(VITA_NP2_PORT)
		if (readlog < 128) {
			OEMSPRINTF(logbuf, OEMTEXT("hdd_read: prepare fail drv=%02x pos=%ld size=%u"), sxsi->drv, logpos, logsize);
			vita_np2_log(logbuf);
			readlog++;
		}
#endif
...
#if defined(VITA_NP2_PORT)
		if (readlog < 128) {
			OEMSPRINTF(logbuf, OEMTEXT("hdd_read: range fail drv=%02x pos=%ld size=%u total=%ld"), sxsi->drv, logpos, logsize, sxsi->totals);
			vita_np2_log(logbuf);
			readlog++;
		}
#endif
...
#if defined(VITA_NP2_PORT)
		if (readlog < 128) {
			OEMSPRINTF(logbuf, OEMTEXT("hdd_read: seek fail drv=%02x sec=%ld off=%ld got=%ld"), sxsi->drv, logpos, pos, r);
			vita_np2_log(logbuf);
			readlog++;
		}
#endif
...
#if defined(VITA_NP2_PORT)
			if (readlog < 128) {
				OEMSPRINTF(logbuf, OEMTEXT("hdd_read: file fail drv=%02x sec=%ld chunk=%u"), sxsi->drv, logpos, rsize);
				vita_np2_log(logbuf);
				readlog++;
			}
#endif
...
#if defined(VITA_NP2_PORT)
	char logbuf[160];
	vita_np2_log("sxsihdd_open: begin");
	vita_np2_log((fname) ? fname : "sxsihdd_open: fname=(null)");
#endif
	FILEH		fh;
const OEMCHAR	*ext;
	REG8		iftype;
	...
	if (fh == FILEH_INVALID) {
#if defined(VITA_NP2_PORT)
		vita_np2_log("sxsihdd_open: file_open failed");
#endif
		goto sxsiope_err1;
	}
#if defined(VITA_NP2_PORT)
	vita_np2_log("sxsihdd_open: file_open ok");
#endif
	ext = file_getext(fname);
	iftype = sxsi->drv & SXSIDRV_IFMASK;
#if defined(VITA_NP2_PORT)
	OEMSPRINTF(logbuf, OEMTEXT("sxsihdd_open: iftype=%u ext=%s"), iftype, ext);
	vita_np2_log(logbuf);
#endif
...
#if defined(VITA_NP2_PORT)
		vita_np2_log("sxsihdd_open: unsupported extension/iftype");
#endif
		goto sxsiope_err2;
	}
#if defined(VITA_NP2_PORT)
	OEMSPRINTF(logbuf, OEMTEXT("sxsihdd_open: hdr=%u cyl=%u surf=%u sec=%u size=%u total=%ld"), headersize, cylinders, surfaces, sectors, size, totals);
	vita_np2_log(logbuf);
#endif
...
#if defined(VITA_NP2_PORT)
	OEMSPRINTF(logbuf, OEMTEXT("sxsihdd_open: success media=%u"), sxsi->mediatype);
	vita_np2_log(logbuf);
#endif
	return(SUCCESS);
...
sxsiope_err2:
#if defined(VITA_NP2_PORT)
	vita_np2_log("sxsihdd_open: failure close");
#endif
	file_close(fh);

sxsiope_err1:
#if defined(VITA_NP2_PORT)
	vita_np2_log("sxsihdd_open: failure");
#endif
	return(FAILURE);
}
